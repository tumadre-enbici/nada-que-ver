...
def _parse_version(v):
    """Convierte una cadena de versión en tupla comparable. Reemplaza distutils.version.LooseVersion."""
    try:
...
class Request(object):
    ...
    def _request(self, req_url):
        try:
            response = self._parse_json(urlopen(req_url, timeout=5).read())
...
class Server(Request):
    ...
    def getstream(self, **params):
        ...
        if _parse_version(self.version) < _parse_version('3.1.29'):
            params['sid'] = params.pop('pid', params.get('sid', ''))
    ...
    def version(self):
        ...
        return self._get_response_key(response, 'version') or ''
