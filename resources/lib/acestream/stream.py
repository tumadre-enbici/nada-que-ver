...
class Stream(Extendable, Observable):
    ...
    def start(self, hls=False, **kwargs):
        ...
        if response.success:
            ...
            try:
                response = self.server.getserver(
                    method='get_media_files', api_version=3, infohash=self.infohash
                )
                if response.success:
                ...
                if not self.id:
                    response = self.server.getserver(
                        method='get_content_id', infohash=self.infohash
                    )
                    if response.success:
    ...
    def _check_required_args(self, **kwargs):
        ...
        if not any(values):
            raise TypeError("__init__() missing 1 required positional argument: 'id' or 'url' or 'infohash'")
        if len(values) > 1:
            raise TypeError("__init__() too many positional arguments, provide only one of 'id' or 'url' or 'infohash'")
